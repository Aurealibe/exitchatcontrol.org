...
test('answering every question reveals a 100/100 sovereign score', async ({
  ...
  // client-drawn scorecard preview (data: PNG — CSP-safe, zero third-party)
  const preview = page.locator('[data-quiz-scorecard-preview]')
  await expect(preview).toBeVisible()
  await expect(preview).toHaveAttribute('src', /^data:image\/png/)
  await expect(page.locator('[data-quiz-share="download"]')).toBeVisible()
})
